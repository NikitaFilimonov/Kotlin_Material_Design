...
import com.example.kotlin_material_design.databinding.FragmentSettingsBinding
...
class SettingsFragment : Fragment() {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.marsTheme.apply {
            setOnClickListener {
                if (themeName != MARS) {
                    saveThemeSettings(MARS, R.style.Theme_Mars)
                    activity?.let {
        ...
        binding.spaceTheme.apply {
            setOnClickListener {
                if (themeName != SPACE) {
                    saveThemeSettings(SPACE, R.style.Theme)
                    activity?.let {
        ...
        binding.moonTheme.apply {
            setOnClickListener {
                if (themeName != MOON) {
                    saveThemeSettings(MOON, R.style.Theme_Moon)
                    activity?.let {
    ...
    override fun onCreateView(
        ...
        val resIdTheme =
            requireActivity().getSharedPreferences(SETTINGS_SHARED_PREFERENCE, Context.MODE_PRIVATE)
                .getInt(THEME_RES_ID, R.style.Theme)
